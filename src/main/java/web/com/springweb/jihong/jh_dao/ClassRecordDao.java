package web.com.springweb.jihong.jh_dao;
...
public interface ClassRecordDao {
	...
	List<JoinMembership> studentList(JoinMembership sch);
	/*
	@Select("SELECT MEM_PRID FROM JOIN_MEMBERSHIP WHERE ")
	*/
	...
	int classRecordEditMsg(ClassRecord upt);
	// 특정 recordId로 수업 기록을 찾는 쿼리
    @Select("SELECT * FROM Class_Record WHERE RECORD_ID = #{recordId}")
    ClassRecord findById(int recordId);
}
