  ...
  <body>
    ...
    <main class="main" id="top">
      ...
      <nav class="navbar navbar-expand-lg navbar-light sticky-top py-3 d-block" data-navbar-on-scroll="data-navbar-on-scroll">
        <div class="container"><a class="navbar-brand" href="${pageContext.request.contextPath}/main"><img src="assets/img/gallery/logo-n.png" height="45" alt="logo" /></a>
          ...
          <div class="collapse navbar-collapse border-top border-lg-0 mt-4 mt-lg-0" id="navbarSupportedContent">
            ...
            </ul><a class="btn btn-primary order-1 order-lg-0" href="${pageContext.request.contextPath}/gilogin">Login</a>
	...
	<section class="pb-11 pt-7 bg-600">
	  <div class="container d-flex justify-content-center">
	    <div class="p-5 bg-white rounded-3 shadow" style="max-width: 500px; width: 100%;">
	      <h1 class="mb-4 text-center">Join Membership</h1>
	        ...
	        <div class="mb-3">
         		...
         		<div id="pwdMatch" class="error-msg"></div>
	        </div>
			<div class="mb-3">
			    <label class="form-label">이름</label>
			    <input class="form-control" type="text" name="memName" required/>
			</div>
	        <div class="mb-3">
          		<label class="form-label">핸드폰</label>
          		<input class="form-control" type="tel" name="memPhone" required />
	        </div>
	        <div class="mb-3">
          		<label class="form-label">메일</label>
          		<input class="form-control" type="email" name="memEmail" required />
	        </div>
	        <div class="mb-3">
		        <label class="form-label">주소</label>
		        <input class="form-control" type="text" name="memAddress" required/>
	        </div>
	        <div class="mb-3">
	          	<label class="form-label">생년월일</label>
	         	<input class="form-control" type="date" name="memBirthday" required/>
	        </div>
	        <div class="mb-4">
...
<script>
  ...
  $('#memId').on('blur', function () {
    ...
    $.ajax({
      url: '${path}/checkId', // contextPath 포함
      type: 'post',
      data: { memId: id },
      ...
      error: function () {
        $('#idCheck').text('서버 오류로 중복 확인에 실패했습니다.')
                     .removeClass('valid-msg').addClass('error-msg');
      }
    });
  });

  // submit 차단 및 아이디 중복 메시지 체크
  $('#joinForm').on('submit', function (e) {
    const idMessage = $('#idCheck').text();

    if (idMessage !== '사용 가능한 아이디입니다.') {
      e.preventDefault(); // 폼 제출 차단
      alert('아이디 중복 확인이 필요하거나, 사용할 수 없는 아이디입니다.');
      $('#memId').focus();
      return false;
    }
  ...
  // 유저가 아이디를 바꿀 때마다 다시 중복확인 필요하게 만들기
  $('#memId').on('input', function () {
    $('#idCheck').text('아이디 중복 확인 중...').removeClass('valid-msg error-msg');
  });
</script>
