...
<script type="text/javascript">
	$(document).ready(function(){
		 $("#studentSelect").change(function () {
		      ...
		      if (selectedId) {
		        $("#studentId").text(selectedId);  // 선택된 학생의 ID를 텍스트로 표시
		      } else {
		        $("#studentId").text("-- 선택된 학생의 ID가 여기에 표시됩니다 --"); // 초기 텍스트
		      }
		    });
	  ...
	  <div class="container d-flex justify-content-center">
	    <div class="p-5 bg-white rounded-3 shadow" style="max-width: 500px; width: 100%;">
	      <h1 class="mb-4 text-center" style="font-family: Sans-serif">수업 기록</h1>
     		<form id="classRecordForm" method="post" action="classRecordInsert">
     								
			<div class="mb-3">
				<label class="form-label">학생 ID, 이름</label> 
				<select class="form-select" id="studentName" name="studentName" required>
						
						<c:forEach var="student" items="${studentList}">
							<option value="${student.memPrid}">${student.memPrid} ${student.memName}</option>
						</c:forEach>
					</select>
			</div>
