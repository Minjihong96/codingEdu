...
<script type="text/javascript">
	$(document).ready(function(){
		$(".update-status-btn").click(function() {
		    const btn = $(this);
		    const enrId = btn.data("enrid");

		    $.ajax({
		        url: "${path}/ajaxUpdateStatus",
		        type: "POST",
		        data: { enrId: enrId },
		        success: function(res) {
		            if (res.success) {
		                alert(res.message);
		                const row = $("#row-" + enrId);
		                row.find(".status-cell").text("신청 완료");
		                btn.remove(); // 버튼 제거
		            } else {
		                alert("처리에 실패했습니다.");
		            }
	  ...
	  <nav class="navbar navbar-expand-lg navbar-light sticky-top py-3 d-block" data-navbar-on-scroll="data-navbar-on-scroll">
		...
		<section class="pb-11 pt-7 bg-600">
			...
			<div class="container mt-5">
		        ...
		        <tbody>
					<c:forEach var="enr" items="${enrList}">
						<tr id="row-${enr.enrId}">
							<td>${enr.memPrid}</td>
							<td>${enr.materialsId}</td>
							<td class="status-cell">${enr.status}</td>
							<td><fmt:formatDate value="${enr.enrDate}" pattern="yyyy-MM-dd"/></td>
							<td>
								<c:if test="${enr.status eq '신청 중'}">
									<button class="btn btn-success btn-sm update-status-btn" data-enrid="${enr.enrId}">
										신청 완료 처리
									</button>
								</c:if>
							</td>
