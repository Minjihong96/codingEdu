  ...
  <body>
    ...
    <main class="main" id="top">
        ...
        <div class="container">
          <div class="row">
            <div class="col-12">
              ...
              <h1 class="mb-6 fw-bold" style="font-family:Nanum Gothic;">${mat.title}</h1>
            </div>
          </div>
        </div>
        <!-- end of .container-->
      ...
      <section style="margin-top:-21.5rem">
        ...
        <div class="container">
          <div class="row">
            <div class="col-md-8"><img class="w-75 h-75" src="${path}${mat.materialsImg}" alt="..." />
              <h1 class="my-4 fw-bold" style="font-family:Nanum Gothic;">Description</h1>
              <p style="font-family:Nanum Gothic;">${mat.description}</p>
            ...
            <div class="col-md-4">
              <div class="card">
                <ul class="list-group list-group-flush">
                  <li class="list-group-item"><strong>Duration </strong>: ${mat.duration} hours</li>
                  <li class="list-group-item"><strong>Lectures </strong>: ${mat.lectures}</li>
                  <li class="list-group-item"><strong>Level </strong>: ${mat.difficulty}</li>
                  <li class="list-group-item"><strong>price </strong>: ${mat.price}</li>
                  <li class="list-group-item text-center"><img src="assets/img/gallery/searching.png" width="78" alt="..."></li>
				  <li class="list-group-item text-center">
				  
					<c:choose>
					    <c:when test="${status eq '신청 완료'}">
					      <a href="${path}/download?filePath=${mat.filePath}" class="btn btn-success w-100" download>강의 다운로드</a>
					    </c:when>
					    <c:when test="${status eq '신청 중'}">
					      <form action="${path}/pconfirmEnr" method="get" style="margin-bottom: 0.5rem;">
					        <input type="hidden" name="memPrid" value="${sessionScope.loginUser.memPrid}" />
					        <input type="hidden" name="materialsId" value="${mat.materialId}" />
					        <button type="submit" class="btn btn-warning w-100">신청 확인</button>
					      </form>
					      <form id="delete" action="${path}/pcancelEnr" method="post">
					        <input type="hidden" name="memPrid" value="${sessionScope.loginUser.memPrid}" />
					        <input type="hidden" name="materialsId" value="${mat.materialId}" />
					        <button id="delBtn" type="button" class="btn btn-outline-danger w-100">수강 취소</button>
					      </form>
					    </c:when>
						<c:otherwise>
						  <form id="insert" action="${path}/pinsertEnr" method="post">
						    <input type="hidden" name="memPrid" value="${sessionScope.loginUser.memPrid}" />
						    <input type="hidden" name="materialsId" value="${mat.materialId}" />
						    <input type="hidden" name="status" value="신청 중" />
						    <button id="regBtn" type="button" class="btn btn-primary w-100">수강 신청</button>
						  </form>
						</c:otherwise>
					</c:choose>
				  </li>
