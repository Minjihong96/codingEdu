...
<script type="text/javascript">
	$(document).ready(function(){
    	$("#uptBtn").click(function(){
			if(confirm("수정하시겠습니까?")) {
				$("#memberInfoForm").attr("action","stupdateMem")
				$("#memberInfoForm").submit()
			}
		})
		$("#delBtn").click(function(){
		    $('#withdrawalModal').modal('show'); // Bootstrap 모달 열기
		});
	});
</script>
...
<form id="memberInfoForm" method="post"> <%-- 회원 정보 조회 또는 수정 폼 --%>
    ...
    <div class="text-center mt-4">
    	<button class="btn btn-danger btn-sm" type="button" id="delBtn">회원 탈퇴</button>
	</div>
</form>
<!-- 탈퇴 사유 입력 모달 -->
<div class="modal fade" id="withdrawalModal" tabindex="-1" aria-labelledby="withdrawalModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <form id="withdrawalForm" method="post" action="${path}/stdeleteMem">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="withdrawalModalLabel">회원 탈퇴 이유</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="닫기"></button>
        </div>
        <div class="modal-body">
		  <input type="hidden" name="memId" value="${mem.memId}">
          <input type="hidden" name="memPrid" value="${mem.memPrid}">
          <input type="hidden" name="memName" value="${mem.memName}">
          <div class="mb-3">
            <label for="reason" class="form-label">탈퇴 이유를 입력해 주세요</label>
