...
<script type="text/javascript">
	$(document).ready(function(){
		let msg = "${msg}"
			if(msg!=""){
				if(!confirm(msg+"\n계속 등록하시겠습니까?")){
					location.href="insertSubmission"
				}
			}
...
<!-- 
 ...
 -->
	  ...
	  <div class="container d-flex justify-content-center">
	    <div class="p-5 bg-white rounded-3 shadow" style="max-width:700px; width:100%;">
	      <h1 class="mb-4 text-center" style="font-family: sans-serif;">과제 제출 폼</h1>
             <form method="post" action="insertSubmission" enctype="multipart/form-data">
    <input type="hidden" name="assignmentId" value="${assignment.assignmentId}" />
    <input type="hidden" name="memPrid" value="${loginUser.memPrid}" />  
      
                <div class="mb-3">
	          	<label for="content" class="form-label">과제내용</label>
          		<textarea class="form-control" name="content" id="content"  rows="13" placeholder="과제 내용 입력" required>과제</textarea>
	        </div>
             <div class="mb-3">
                  <label for="report" class="form-label">파일첨부</label>
                  <input class="form-control" type="file" name="report" id="report" >
                </div>
                   <div class="text-center mt-4">
	          <button class="btn btn-primary w-100" id="regBtn" type="submit">제출하기</button>
	        </div>
	           </form>
           </div>   
