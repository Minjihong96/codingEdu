  ...
  <body>
    ...
    <main class="main" id="top">
	  ...
	  <h1 class="mb-4 mt-5 text-center" style="font-family: sans-serif;">회원 정보</h1>
          ...
          --%>
          <c:if test="${not empty mem}"> <%-- member 객체가 Model에 있다면 (로그인된 상태 등) --%>
    ...
    <div class="form-group-custom">
        ...
        <input type="text" class="form-control" id="memBirthday" name="memBirthday" value="<fmt:formatDate value="${mem.memBirthday}" pattern="yyyy-MM-dd"/>" placeholder="YYYY-MM-DD" required>
    </div>

    <div class="form-group-custom">
        <label for="accountType" class="form-label">가입 유형</label>
        <input type="text" class="form-control" id="accountType" name="accountType" value="${mem.accountType}" readonly>
    </div>

    <div class="form-group-custom">
        <label for="createdAt" class="form-label">가입 일자</label>
        <input type="text" class="form-control" id="createdAt" name="createdAt" value="<fmt:formatDate value="${mem.createdAt}" pattern="yyyy-MM-dd HH:mm"/>" readonly>
    </div>

    <div class="text-center mt-4">
        <button class="btn btn-primary" type="button" id="updateMemberBtn">회원 정보 수정</button>
        <%-- 필요에 따라 다른 버튼(예: 목록으로 돌아가기)을 추가할 수 있습니다. --%>
    </div>
    <div class="text-center mt-3">
    <button class="btn btn-danger" type="button" id="deleteMemberBtn">회원 탈퇴</button>
</div>
