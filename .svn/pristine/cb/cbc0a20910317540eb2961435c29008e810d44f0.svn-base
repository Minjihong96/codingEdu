      ...
      <nav class="navbar navbar-expand-lg navbar-light sticky-top py-3 d-block" data-navbar-on-scroll="data-navbar-on-scroll">
        ...
        <c:when test="${not empty sessionScope.loginUser}">
            <li class="nav-item px-2">
                <a class="nav-link" href="${path}/member/myCourses">나의 수강목록</a>
            </li>
            <li class="nav-item px-2">
                <a class="nav-link" href="${path}/member/myPage">회원정보확인</a>
            </li>
        </c:when>
        ...
        <c:otherwise>
            <li class="nav-item px-2">
                <a class="nav-link" href="${path}/member/student">student</a>
            </li>
        </c:otherwise>
    </c:choose>
...
<div class="d-flex"> <%-- 버튼을 감싸는 div는 예시입니다. --%>
    <c:choose>
        <%-- CASE 1: 로그인 한 경우 --%>
        <c:when test="${not empty sessionScope.loginUser}">
            <a class="btn btn-primary" href="${pageContext.request.contextPath}/logout.do">Logout</a>
        </c:when>

        <%-- CASE 2: 로그인 하지 않은 경우 --%>
        <c:otherwise>
            <a class="btn btn-primary" href="${pageContext.request.contextPath}/gilogin">Login</a>
        </c:otherwise>
    </c:choose>
</div>
